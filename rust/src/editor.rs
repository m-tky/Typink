...
impl HeadlessEditor {
    ...
    pub fn char_idx_to_utf16_idx(&self, char_idx: usize) -> usize {
        ...
        u16_idx += line.chars().take(chars_in_target_line).map(|c| c.len_utf16()).sum::<usize>();
    ...
    pub fn utf16_idx_to_byte_idx(&self, u16_idx: usize) -> usize {
        ...
        self.buffer.char_to_byte(char_idx.min(self.buffer.len_chars()))
    }
    ...
    pub fn replace_range(&mut self, start_u16: usize, end_u16: usize, text: &str, cursor_u16: Option<usize>) {
        ...
        if self.history.last().map(|h| &h.0) != Some(&current.0) {
    ...
    pub fn handle_key(&mut self, key: &str) -> Option<VimAction> {
        ...
        let old_content = if key == "u" || key == "\x12" { None } else { Some((self.buffer.to_string(), prev_line, prev_col)) };
        ...
        if let Some(a) = action {
            ...
            if a.delete_range.is_some() || a.insert_text.is_some() {
                if let Some(orig) = old_content {
                    if self.history.last().map(|h| &h.0) != Some(&orig.0) {
    ...
    pub fn get_view(&mut self, start_line: usize, end_line: usize) -> EditorView {
        ...
        for i in start_line..end {
            ...
            let first_span_idx = self.cached_spans.binary_search_by_key(&start_u16, |s| s.end).unwrap_or_else(|e| e);
        ...
        EditorView {
            ...
            search_query: if self.vim.mode == VimMode::Search { Some(self.vim.command_buffer.clone()) } else { self.vim.last_search.clone() },
    ...
    pub fn undo(&mut self) {
        if let Some((content, line, col)) = self.history.pop() {
            self.redo_stack.push((self.buffer.to_string(), self.vim.line, self.vim.col));
            self.buffer = Rope::from_str(&content);
            self.vim.line = line;
            self.vim.col = col;
            self.vim.snap_cursor_to_valid(&self.buffer);
    ...
    pub fn redo(&mut self) {
        if let Some((content, line, col)) = self.redo_stack.pop() {
            self.history.push((self.buffer.to_string(), self.vim.line, self.vim.col));
            self.buffer = Rope::from_str(&content);
            self.vim.line = line;
            self.vim.col = col;
            self.vim.snap_cursor_to_valid(&self.buffer);
    ...
    pub fn get_completions(&mut self, content: String, offset_u16: usize) -> Vec<crate::api::TypstCompletion> {
        ...
        completions.map(|(_, items)| {
            items.into_iter().map(|item| {
                crate::api::TypstCompletion {

...
pub fn highlight_typst(content: &str) -> Vec<HighlightSpan> {
    ...
    traverse_and_collect(&root, &mut spans, &byte_to_u16, None, false, false, None, 0, &mut bracket_depth);
...
fn traverse_and_collect(
    ...
    let in_math = kind == typst_syntax::SyntaxKind::Equation || (parent_tag.as_ref().map(|t| t.starts_with("math")).unwrap_or(false));
    ...
    let is_opening = matches!(kind, typst_syntax::SyntaxKind::LeftParen | typst_syntax::SyntaxKind::LeftBracket | typst_syntax::SyntaxKind::LeftBrace);
    let is_closing = matches!(kind, typst_syntax::SyntaxKind::RightParen | typst_syntax::SyntaxKind::RightBracket | typst_syntax::SyntaxKind::RightBrace);
    ...
    match kind {
        ...
        typst_syntax::SyntaxKind::Heading => {
            if let Some(marker) = node.children().find(|c| c.kind() == typst_syntax::SyntaxKind::HeadingMarker) {
        ...
        for c in text.chars() {
            ...
            if is_op || is_cl {
                ...
                if char_offset > last_pushed_offset {
                    if effective_tag.is_some() || current_heading_level.is_some() {
    ...
    for child in node.children() {
        traverse_and_collect(&child, spans, byte_to_u16, effective_tag.clone(), is_bold, is_italic, current_heading_level, depth + 1, bracket_depth);
    }
...
fn normalize_tag(tag: typst_syntax::Tag, in_math: bool) -> String {
    let s = format!("{:?}", tag);
    let mut label = if s.contains("Heading") { "heading".to_string() }
    else if s.contains("Math") { "math".to_string() }
    else if s.contains("Function") || s.contains("Decorator") { "function".to_string() }
    else if s.contains("Variable") { "variable".to_string() }
    else if s.contains("Keyword") { "keyword".to_string() }
    else if s.contains("String") { "string".to_string() }
    else if s.contains("Comment") { "comment".to_string() }
    else if s.contains("Operator") { "operator".to_string() }
    else if s.contains("Punctuation") { "punctuation".to_string() }
    else if s.contains("Raw") { "raw".to_string() }
    else if s.contains("Label") { "label".to_string() }
    else if s.contains("Ref") { "ref".to_string() }
    else if s.contains("Url") { "link".to_string() }
    else if s.contains("Marker") { "marker".to_string() }
    else if s.contains("Error") { "error".to_string() }
    else { s.to_lowercase() };
...
mod tests {
    ...
    fn test_highlighting() {
        ...
        for span in &spans {
            ...
            println!("Span: [{}, {}] -> label: {}", span.start, span.end, span.label);
        }
        
        let h1_spans: Vec<_> = spans.iter().filter(|s| s.heading_level == Some(1)).collect();
        assert!(!h1_spans.is_empty(), "Level 1 heading not found");
        
        let h2_spans: Vec<_> = spans.iter().filter(|s| s.heading_level == Some(2)).collect();
        assert!(!h2_spans.is_empty(), "Level 2 heading not found");

        let math_item_spans: Vec<_> = spans.iter().filter(|s| s.label.starts_with("math.")).collect();
